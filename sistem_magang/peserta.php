...
if (isset($_POST['verify_registration']) && $role == 'admin') {
    ...
    if ($register) {
        ...
        if (mysqli_query($conn, $query)) {
            ...
            $user_nama = mysqli_real_escape_string($conn, $register['nama']);
            $user_email = mysqli_real_escape_string($conn, $register['email']);
            $user_password = $register['password']; // Already hashed from registration
            $user_role = 'user';
            ...
            if (mysqli_query($conn, $user_query)) {
                ...
                $message .= '<br>Akun user dibuat otomatis untuk peserta ini.';
            } else {
                $message .= '<br>Gagal membuat akun user: ' . mysqli_error($conn);
            }
...
// Tambah akun untuk admin/pembimbing
if (isset($_POST['tambah_akun']) && $role == 'admin') {
    $user_nama = mysqli_real_escape_string($conn, $_POST['user_nama']);
    $user_email = mysqli_real_escape_string($conn, $_POST['user_email']);
    $user_password = password_hash($_POST['user_password'], PASSWORD_BCRYPT);
    $user_role = mysqli_real_escape_string($conn, $_POST['user_role']); // 'admin' or 'pembimbing'
    $user_query = "INSERT INTO users (nama, email, password, role, created_at)
                   VALUES ('$user_nama', '$user_email', '$user_password', '$user_role', NOW())";
    if (mysqli_query($conn, $user_query)) {
...
// Update akun
if (isset($_POST['update_akun']) && $role == 'admin') {
    $user_id = (int)$_POST['user_id'];
    $user_nama = mysqli_real_escape_string($conn, $_POST['user_nama']);
    $user_email = mysqli_real_escape_string($conn, $_POST['user_email']);
    $user_role = mysqli_real_escape_string($conn, $_POST['user_role']);
    $update_query = "UPDATE users SET nama = '$user_nama', email = '$user_email', role = '$user_role' WHERE id = $user_id";
    if (isset($_POST['user_password']) && !empty($_POST['user_password'])) {
        $user_password = password_hash($_POST['user_password'], PASSWORD_BCRYPT);
        $update_query = "UPDATE users SET nama = '$user_nama', email = '$user_email', password = '$user_password', role = '$user_role' WHERE id = $user_id";
    }
    if (mysqli_query($conn, $update_query)) {
        $message = 'Akun berhasil diupdate!';
        $messageType = 'success';
    } else {
        $message = 'Gagal mengupdate akun: ' . mysqli_error($conn);
        $messageType = 'error';
    }
}
// Hapus akun
if (isset($_GET['hapus_akun']) && $role == 'admin') {
    $user_id = (int)$_GET['hapus_akun'];
    if ($user_id != $_SESSION['user']['id']) { // Prevent deleting self
...
// Pagination for akun
$search_akun = isset($_GET['search_akun']) ? mysqli_real_escape_string($conn, $_GET['search_akun']) : '';
$page_akun = isset($_GET['page_akun']) ? max(1, (int)$_GET['page_akun']) : 1;
$offset_akun = ($page_akun - 1) * $limit;
$akun_query = "SELECT * FROM users WHERE role IN ('admin', 'pembimbing') AND (nama LIKE '%$search_akun%' OR email LIKE '%$search_akun%') ORDER BY nama LIMIT $limit OFFSET $offset_akun";
$akun = mysqli_query($conn, $akun_query);
$total_akun_query = "SELECT COUNT(*) as total FROM users WHERE role IN ('admin', 'pembimbing') AND (nama LIKE '%$search_akun%' OR email LIKE '%$search_akun%')";
$total_akun_result = mysqli_fetch_assoc(mysqli_query($conn, $total_akun_query));
$total_akun_pages = ceil($total_akun_result['total'] / $limit);
// Update peserta
...
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn <?= isset($_GET['tab']) && $_GET['tab'] == 'tambah_akun' ? 'active' : '' ?>" onclick="showSection('tambah_akun')"><i class="fas fa-user-plus"></i> Tambah Akun</button>
                                ...
                                <table class="table">
                                    ...
                                    <tbody id="verifikasiTable">
                                        ...
                                        <tr>
                                            ...
                                            <td data-label="Aksi">
                                                <div class="btn-group">
                                                    <button type="button" class="btn btn-sm btn-primary" onclick="showRegisterDetail(<?= $r['id'] ?>)">
    ...
    <i class="fas fa-edit"></i>
        ...
        <?php endif; ?>
            ...
            <div id="tambah_akun-section" class="section-content <?= isset($_GET['tab']) && $_GET['tab'] == 'tambah_akun' ? 'active' : '' ?>">
                <?php if ($role == 'admin'): ?>
                    <div class="content-card">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <form method="post">
                                <input type="hidden" name="tab" value="tambah_akun">
                                <div class="mb-3">
                    ...
                    <div class="content-card">
                        <div class="card-header">
                            <h3><i class="fas fa-list"></i> Daftar Akun Admin & Pembimbing</h3>
                        </div>
                        <div class="card-body">
                            <div class="search-container">
                                <form method="get" id="searchAkunForm">
                                    <input type="hidden" name="tab" value="tambah_akun">
                                    <input type="text" name="search_akun" id="searchAkun" placeholder="Cari nama atau email..." value="<?= htmlspecialchars($search_akun) ?>">
                                    <button type="submit"><i class="fas fa-search"></i> Cari</button>
                                </form>
                            </div>
                            <div class="table-container">
                                    ...
                                    <tbody id="akunTable">
                                        ...
                                        while($row = mysqli_fetch_assoc($akun)): ?>
                                            <tr>
                                                ...
                                                <td data-label="Aksi">
                                                    <div class="btn-group">
                                                        <button onclick="showEditAkun(<?= $row['id'] ?>, '<?= addslashes($row['nama']) ?>', '<?= addslashes($row['email']) ?>', '<?= addslashes($row['role']) ?>')"
                                                                class="btn btn-warning btn-sm">
                                                            <i class="fas fa-edit"></i> Edit
                                                        </button>
                                                        <a href="?tab=tambah_akun&hapus_akun=<?= $row['id'] ?>"
                            ...
                            <div class="pagination">
                                <?php if ($page_akun > 1): ?>
                                    <a href="?tab=tambah_akun&page_akun=<?= $page_akun - 1 ?>&search_akun=<?= urlencode($search_akun) ?>"><i class="fas fa-chevron-left"></i> Sebelumnya</a>
                                <?php else: ?>
                                    <span><i class="fas fa-chevron-left"></i> Sebelumnya</span>
                                <?php endif; ?>
                                <span>Halaman <?= $page_akun ?> dari <?= $total_akun_pages ?></span>
                                <?php if ($page_akun < $total_akun_pages): ?>
                                    <a href="?tab=tambah_akun&page_akun=<?= $page_akun + 1 ?>&search_akun=<?= urlencode($search_akun) ?>">Selanjutnya <i class="fas fa-chevron-right"></i></a>
                                <?php else: ?>
                                    <span>Selanjutnya <i class="fas fa-chevron-right"></i></span>
                                <?php endif; ?>
                            </div>
        ...
        <div class="modal-body">
            ...
            <div id="editAkunModal" class="modal">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4>Edit Akun</h4>
                        <span class="close" onclick="closeModal('editAkunModal')">×</span>
                    </div>
                    <div class="modal-body">
                        <form method="post">
                            <input type="hidden" name="tab" value="tambah_akun">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="edit_user_nama" name="user_nama" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="edit_user_password" class="form-label">Password Baru (kosongkan jika tidak diubah)</label>
                            ...
                            <button type="submit" name="update_akun" class="btn btn-primary">
                                <i class="fas fa-save"></i> Simpan Perubahan
                            </button>
                        </form>
                    </div>
...
function showRegisterDetail(id) {
    console.log('showRegisterDetail called with id:', id);
    $.ajax({
        url: 'api/get_register.php',
        method: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function(data) {
            ...
            const html = `
                <div class="text-center mb-3">
                    <img src="${data.foto ? data.foto : 'https://via.placeholder.com/150'}" alt="Foto Registrasi" style="width:150px; height:150px; border-radius:8px; object-fit:cover;">
                </div>
                <p><strong>Nama:</strong> ${data.nama || '-'}</p>
                <p><strong>Email:</strong> ${data.email || '-'}</p>
                ...
                <p><strong>Tanggal Registrasi:</strong> ${data.created_at ? new Date(data.created_at).toLocaleString('id-ID') : '-'}</p>
            `;
            $('#detailContent').html(html);
            document.getElementById('detailModal').style.display = 'block';
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('Register Detail AJAX error:', textStatus, errorThrown, 'Status:', jqXHR.status, 'Response:', jqXHR.responseText);
            $('#detailContent').html('<p class="alert alert-error">Gagal memuat data registrasi: ' + textStatus + '</p>');
            document.getElementById('detailModal').style.display = 'block';
        }
    });
}
...
function showEditAkun(id, nama, email, role) {
    $('#edit_user_id').val(id);
    $('#edit_user_nama').val(nama);
    $('#edit_user_email').val(email);
    $('#edit_user_role').val(role);
    $('#edit_user_password').val('');
    document.getElementById('editAkunModal').style.display = 'block';
}
...
$(document).ready(function() {
    ...
    const debouncedSearchAkun = debounce(function() {
        performSearch($('#searchAkun'), 'akunTable', 'api/search_akun.php', 'tambah_akun');
    }, 300);
    ...
    $('#searchAkun').on('input', debouncedSearchAkun);
